...
const UpdateContato = () => {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:8080/contatos/${contatoId}`)
      .then((response) => {
  ...
  const handleInputChange = (e) => {
    if (e.target.name == "clienteId") {
      setContato((contato.cliente.id = e.target.value));
    }
  ...
  const handleUpdateContato = () => {
    axios
      .put(
        `http://localhost:8080/contatos/${contatoId}/${contato.cliente.id}`,
        contato
      )
      .then((response) => {
