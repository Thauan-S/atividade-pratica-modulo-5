...
const ContatoCriar = () => {
    ...
    const [contato,setContato]=useState({assunto:"",mensagem:"",cliente:{id:""}})
    ...
    const handleAddContato=()=>{
        axios
        .post(`http://localhost:8080/contatos/${clienteid}`,contato)
        .then((response)=>{
  ...
  <div className="form-floating  col-2 col-md-1">
    <input
      ...
      id="floatingTextarea"
      name='idCliente'
      value={contato.cliente.id=codigo}
    />
  ...
  <div style={{marginLeft:"1px"}} className=" form-floating mt-4 ml-3  row">
          
          <textarea   className="form-control "  value={contato.mensagem} name="mensagem" onChange={handleInputChange} className="col-11 col-sm-5" rows="3" id="textarea"></textarea>
          </div>
  <button
